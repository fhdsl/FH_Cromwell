...
This allows us to simplify our workflow testing and design, leverage this tool for smaller scale work that does not need the cloud, and can let users of all kinds manage their workflow work over time via this tool.  

## Pre-Requisites
Either you need to have gotten access to use the Fred Hutch SLURM cluster prior OR have taken our "Cluster 101" course.  


## What is Cromwell?
Cromwell is a workflow manager developed by the Broad which manages the individual tasks involved in multi-step workflows, tracks job metadata, provides an API/Swagger UI interface and allows users to manage multiple workflows simultaneously.  
Cromwell isn't the only WDL "runner" that exists, but it is a tool that has been configured for use on the Fred Hutch gizmo cluster in order to make running workflows here very simple.


## Steps to prepare
If you have questions about these steps, feel free to email Amy Paguirigan (`apaguiri`) or `scicomp`.  
