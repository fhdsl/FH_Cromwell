...
1.  Log into Rhino
2.  Go to your `cromwell-home` directory, and kick off a server job using the command: `./cromwell.sh myConfig.txt`
3.  Wait for a successful response and the node:port information for your server!

That's it!  Now your Cromwell server will run for a week by default (unless you have set a different server length in `myConfig.txt`).  It will be accessible to submit workflows to and execute them whenever you want through mulitple mechanisms that we'll describe in the next chapters.  Next week you can simply repeat the above to restart your server and it'll be ready again!  
